...
setwd("~/Sophieguo/Projects/MouseExperiment/B6Young-old/RNAseq-1-2-3")
...
#4-24-2019; match with human AD genes 

setwd("~/Sophieguo/Projects/MouseExperiment/basicresearch.discussionApril2019/AL.RNAseq/Eric M.Blalock.humanAD")

DAVID.geneEntrez<-read.delim("DAVID.geneEntrez.txt",sep="\t",header=T,stringsAsFactors = F)

HOM_human<-read.delim("HOM_MouseHumanSequence.4.2019.txt",sep="\t",header=T,stringsAsFactors = F)%>%
  filter(Common.Organism.Name=="human")

HOM_MtoH<-read.delim("HOM_MouseHumanSequence.4.2019.txt",sep="\t",header=T,stringsAsFactors = F)%>%
  filter(Common.Organism.Name!="human")%>%
  left_join(.,HOM_human,by="HomoloGene.ID",suffix=c(".mouse",".human"))%>%
  dplyr::select(HomoloGene.ID,starts_with("Symbol"),starts_with("Entrez"))%>%
  distinct(Symbol.human,.keep_all=T)


LCMHP.humanAD.all<-read.delim("LCM.HP.all.sup1.txt",sep="\t",header=T,stringsAsFactors = F)%>%
  left_join(.,DAVID.geneEntrez,by="Probe.set")%>%
  dplyr::select(-starts_with("X"))%>%
  rename(Symbol.human=Symbol)%>%
  left_join(.,HOM_MtoH,by="Symbol.human")%>%
  mutate(class=ifelse(Incip..MMSE<0.05|Incip..NFT<0.05,"atleastonesig","ns"))


LCMHP.mouseID<-LCMHP.humanAD.all%>%
  filter(!is.na(Symbol.mouse))%>%
  mutate(negr.MMSE.all=-r.MMSE.all,negr.MMSE.incip=-r.MMSE.incip)%>%
  mutate(aver.all=rowMeans(select(.,r.NFT.all,negr.MMSE.all)),aver.incip=rowMeans(select(.,r.NFT.incip,negr.MMSE.incip)))%>%
  mutate(dir.all=ifelse(aver.all>0,"up.AD","down.AD"))%>%
  mutate(class=ifelse(p.MMSE.incip<0.05|p.NFT.incip<0.05,"incip","non-incip"))%>%
  mutate(dir.incip=ifelse(class=="incip"&aver.incip>0,"up.incip",ifelse(class=="incip"&aver.incip<0,"down.incip","non-incip")))


a<-intersect(DEG.combine.all[DEG.combine.all$FDR.Y<0.01,]$geneSymbol,LCMHP.mouseID$Symbol.mouse)

library(VennDiagram)

venn.diagram(list(DEG.combine.all[DEG.combine.all$FDR.Y<0.01,]$geneSymbol, LCMHP.mouseID$Symbol.mouse), "humanAD_DEG.Y_all.tiff", fill=brewer.pal(4,"Set1")[3:4], cex = 2, main="All DEGs: humanAD vs. SDvSS.Y", main.pos=c(0.5,0.64), cat.fontface=2, margin=0.8,main.cex=2, cat.cex=2,category.names=c("SDvSS.Y","humanAD"),cat.pos=c(-30,35))

venn.diagram(list(DEG.combine.all[DEG.combine.all$FDR.Y<0.01,]$geneSymbol, LCMHP.mouseID[LCMHP.mouseID$dir.incip!="non-incip",]$Symbol.mouse), "humanAD_DEG.Y_incip.tiff", fill=brewer.pal(4,"Set1")[3:4], cex = 2, main="All DEGs: humanAD.incip vs. SDvSS.Y", main.pos=c(0.5,0.64), cat.fontface=1.5, margin=0.8,main.cex=2, cat.cex=2,category.names=c("SDvSS.Y","humanAD.incip"),cat.pos=c(-30,10))



overlap.YhumanAD<-LCMHP.mouseID%>%
  dplyr::select(Symbol.mouse,dir.all,dir.incip)%>%
  rename(geneSymbol=Symbol.mouse)%>%
  left_join(data.frame(geneSymbol=intersect(DEG.combine.all[DEG.combine.all$FDR.Y<0.01,]$geneSymbol,LCMHP.mouseID$Symbol.mouse)),.,by="geneSymbol")%>%
  left_join(.,DEG.combine.all[,c(5,12,34)],by="geneSymbol")%>%
  mutate(dir.SDvSS=ifelse(aveFC.SDvSS.Y<0,"down.SD","up.SD"))%>%
  mutate(group=ifelse(dir.all=="down.AD"&dir.SDvSS=="down.SD","g1.dd",ifelse(dir.all=="up.AD"&dir.SDvSS=="down.SD","g2.du",ifelse(dir.SDvSS=="up.SD"&dir.all=="up.AD","g3.uu","g4.ud"))))%>%
  mutate(group.insip=ifelse(dir.incip=="down.incip"&dir.SDvSS=="down.SD","g1.dd",ifelse(dir.incip=="up.incip"&dir.SDvSS=="down.SD","g2.du",ifelse(dir.SDvSS=="up.SD"&dir.incip=="up.incip","g3.uu",ifelse(dir.incip=="non-incip","non-incip","g4.ud")))))

write.table(overlap.YhumanAD,"overlap.YhumanAD.txt",sep="\t",row.names = F)



#match with AD differentially regulated gene 4-16-19
nature.id<-read.delim("nature14252-s1.txt",sep="\t",header=T,stringsAsFactors = F)%>%
  rename(id=Ensembl.ID)%>%
  mutate(Cluster=factor(Cluster,unique(Cluster)))%>%
  mutate(dir=ifelse(grepl("Increase",Cluster),"up.p25",ifelse(grepl("Decrease",Cluster),"down.p25","na")))


venn.diagram(list(DEG.combine.all[DEG.combine.all$FDR.Y<0.01,]$id,nature.id[nature.id$dir!="na",]$id), "nature_DEG.Y_all.tiff", fill=brewer.pal(4,"Set1")[3:4], cex = 2, main="All DEGs: Nature vs. SDvSS.Y", main.pos=c(0.5,0.64), cat.fontface=2, margin=0.8,main.cex=2, cat.cex=2,category.names=c("SDvSS.Y","P25vP35"),cat.pos=c(-30,35))


overlap.Ynature<-data.frame(id=intersect(DEG.combine.all[DEG.combine.all$FDR.Y<0.01,]$id,nature.id[nature.id$dir!="na",]$id))%>%
  left_join(.,nature.id,by="id")%>%
  left_join(.,DEG.combine.all[,c(1,12,34)],by="id")%>%
  mutate(dir.SDvSS=ifelse(aveFC.SDvSS.Y<0,"down.SD","up.SD"))%>%
  mutate(group=ifelse(dir=="down.p25"&dir.SDvSS=="down.SD","g1.dd",ifelse(dir=="up.p25"&dir.SDvSS=="down.SD","g2.du",ifelse(dir.SDvSS=="up.SD"&dir=="up.p25","g3.uu","g4.ud"))))

write.table(overlap.Ynature,"overlap.Ynature.txt",sep="\t",row.names = F)

venn.diagram(list(DEG.combine.all[DEG.combine.all$FDR.SS.OvY<0.01,]$id,nature.id[nature.id$dir!="na",]$id), "nature_DEG.OvY_all.tiff", fill=brewer.pal(4,"Set1")[3:4], cex = 2, main="All DEGs: Nature vs. SDvSS.Y", main.pos=c(0.5,0.64), cat.fontface=2, margin=0.8,main.cex=2, cat.cex=2,category.names=c("OvY.SS","P25vP35"),cat.pos=c(-30,35))


overlap.age.nature<-data.frame(id=intersect(DEG.combine.all[DEG.combine.all$FDR.SS.OvY<0.01,]$id,nature.id[nature.id$dir!="na",]$id))%>%
  left_join(.,nature.id,by="id")%>%
  left_join(.,DEG.combine.all[,c(1,14,36)],by="id")%>%
  mutate(dir.OvY=ifelse(aveFC.OvY.SS<0,"down.O","up.O"))%>%
  group_by(dir.OvY,dir)%>%
  summarise(n())


#4/2/19: query DEGs common to SDvsSS and OvsY; for BiP aging project

DEG.both<-DEG.combine.all%>%
  mutate(DEG.SDvSS.YO=ifelse(FDR.Y<0.01|FDR.O<0.01,"eitherYO","NS"))%>%
  filter(DEG.SDvSS.YO=="eitherYO"&FDR.SS.OvY<0.01)

write.table(DEG.both,"DEG.both.txt",sep="\t",row.names = F)
...
scMarker.DEGboth<-scMarker.id%>%  
  mutate(geneSymbol=gene)%>%
  left_join(DEG.both[,c(1,3,5,12:16,33:37)],.,by="geneSymbol")%>%
...
counts <-read.delim("master_list_of_gene_counts_MIN.sense.UNNORMQUANT-stranded.txt", row.names = 1,stringsAsFactors=FALSE)

#creat a "id" column that contains the ensembl gene id
counts<-counts%>%
  rownames_to_column("id")

count_matrix<-as.matrix(counts[,2:109]) #creat a matrix that contains the gene count data

count_gene<-counts[,c(1,110,111)] #creat a data frame that contains the gene info
...
MouseIDs<-getBM(attributes = c("ensembl_gene_id","entrezgene","description","gene_biotype","transcript_length"),
                mart=mouseensembl)
...
count_gene<-left_join(count_gene,MouseIDs,by="id") #add more annotation to the gene id
...
count_sample<-as.data.frame(sampleID)
...
count_sample<-count_sample%>%
  ...
  mutate(age_sleep_time=paste(age_sleep,timepoint,sep="."))%>%
  mutate(age_sleep_time=factor(age_sleep_time,levels=c("Y.Z0.Z0","Y.SS.3hr","Y.SS.6hr","Y.SS.9hr","Y.SS.12hr","Y.SD.3hr","Y.SD.6hr","Y.SD.9hr","Y.SD.12hr","O.Z0.Z0","O.SS.3hr","O.SS.6hr","O.SS.9hr","O.SS.12hr","O.SD.3hr","O.SD.6hr","O.SD.9hr","O.SD.12hr")))
  
x <- DGEList(counts=count_matrix, samples=count_sample, genes=count_gene) #creat DGElist object x
...
design <- model.matrix(~0+x$samples$age_sleep_time)
colnames(design) <- levels(x$samples$age_sleep_time)
...
png(filename = "MDS2d.Y2.png",height =4,width =7,res = 300,units = "in",type="cairo")
...
#5-29-19; remake TIFF figures for publication

#separate Y from O for the 2D MDS plot
tiff(filename = "Figure1a.tiff",height =55,width =105,res = 800,units = "mm",type="cairo",compression="none")

ggscatter(toplot, x="Dim1",y="Dim2",color="Group",shape="Age",size=1.2,facet.by="Age", palette=palette,ellipse=T, ellipse.alpha = 0.2,xlab="MDS dim1", ylab="MDS dim2") +
theme(legend.title=element_blank(),legend.position="right",panel.background = element_rect(fill="white", linetype="solid", color="black"), plot.title = element_text(face="bold",hjust=-0.1), strip.text=element_text(size = 12, face="bold"),axis.title=element_text(size = 12),axis.text=element_text(size=6))+scale_shape(guide=FALSE)

dev.off()

tiff(filename = "Figure1b.tiff",height =60,width =75,res = 800,units = "mm",type="cairo",compression="none")

scatterplot3d(toplot[,1:3], pch = shapes, cex.symbols=0.8,color=colors, grid=F,angle = -15, cex.lab=1,cex.axis=0.4,xlab = "MDS dim1",ylab = "MDS dim3",zlab = "MDS dim2",mar=c(3,2,1,3))
...
#5-29-19; redo for publication
venn.diagram(list(DEG.combine.all[DEG.combine.all$FDR.Y<0.01,]$id,DEG.combine.all[DEG.combine.all$FDR.O<0.01,]$id), "Figure1c.tiff", fill=brewer.pal(4,"Set1")[3:4], cex = 3, main="", main.pos=c(0.5,0.64), cat.fontface=2, margin=0.8,main.cex=3, cat.cex=3,category.names=c("Young","Old"),cat.pos=c(-50,55))


#volcano plot (5/8/19; version to compare with LCM.Gal data)

png(filename = "volcano.SDvSS.mPFCY.png",height = 4.5,width =5,res = 300,units = "in",type="cairo")

DEG.combine.all%>%
  mutate(group.volcano=ifelse(FDR.Y>=0.05,"NS",ifelse(aveFC.SDvSS.Y>0,"up","down")))%>%
  mutate(logFDR=-log10(FDR.Y))%>%
  ggscatter(.,x="aveFC.SDvSS.Y",y="logFDR",color="group.volcano",palette=c("green4","grey50","red3"),size=1.5,label="geneSymbol",label.select=list(criteria="`x`>0&`y`>32|`x`<0&`y`>25|`x`>2.2"),repel=T,xlim=c(-3.5,3.5),ylim=c(0,41),ylab=expression('-log'[10]*'FDR'),xlab=expression('Average log'[2]*'fold change of SD vs. SS'),title="DEGs of SD vs.SS in mPFC (bulk)")+
  geom_hline(yintercept=1.3,linetype="dashed",color="grey")+
  geom_vline(xintercept=0,linetype="dashed",color="grey")+
  annotate("text", x = c(-3,-3), y = c(40,36), label = c("3444", "3637"), color=c("green4","red"), size=8, fontface="bold")+
  annotate("segment", x = -2.1, xend = -2.1, y = 41.5, yend = 38.5, colour = "green4", size=2, alpha=1, arrow=arrow(length = unit(0.3, "cm")))+
  annotate("segment", x = -2.1, xend =-2.1, y = 34.5, yend =37, colour = "red", size=2, alpha=1, arrow=arrow(length = unit(0.3, "cm")))+
...
scale_x_continuous(breaks=c(-3,-2,-1,0,1,2,3))+scale_y_continuous(breaks=c(1.3,10,20,30,40))
...
ggarrange(vol.Y,vol.O,ncol=2,labels = c("(d)",""))
 ...
 #5-29-19; recreate TIFF figures for publication
tiff(filename = "Figure1d.tiff",height =60,width =60,res = 800,units = "mm",type="cairo",compression="none")

vol.Y<-DEG.combine.all%>%
  mutate(class.y=ifelse(FDR.Y>0.01,"ns",ifelse(abs(aveFC.SDvSS.Y)>0.2,"high","low")),class.o=ifelse(FDR.O>0.01,"ns",ifelse(abs(aveFC.SDvSS.O)>0.2,"high","low")))%>%
  mutate_at(vars(starts_with("FDR")),funs(-log10(.)))%>%
ggscatter(.,x="aveFC.SDvSS.Y",y="FDR.Y",size=0.4,color="class.y", palette=c("green3","gray50","gray80"),xlab=expression('log'[2]*'FC of SD vs.SS'), ylab=expression('-log'[10]*'FDR'),title="", xlim=c(-3.5,3.5))+
geom_hline(yintercept=2, linetype="longdash", color = "black",size=0.5)+
  geom_vline(xintercept=c(-0.2,0.2), linetype="longdash", color = c("red","blue"),size=0.5)+
  scale_x_continuous(breaks=c(-3,-2,-1,-0.2,0.2,1,2,3))+scale_y_continuous(breaks=c(0,2,10,20,30))+
  theme(legend.position = "none",axis.title=element_text(size=10), axis.text.x=element_text(angle = 60,hjust = 1,size=6),axis.text.y=element_text(size=6))+
  annotate("segment", x = -0.2, xend = -1.2, y = 30, yend = 30, colour = "red", size=0.5, alpha=0.6, arrow=arrow(length = unit(0.2, "cm")))+
  annotate("segment", x = 0.2, xend = 1.2, y = 30, yend = 30, colour = "blue", size=0.5, alpha=0.6, arrow=arrow(length = unit(0.2, "cm")))+
  annotate("text", x = c(-2.5,2.5), y = c(30,30), label = c("1858", "1944") , color=c("red","blue"), size=4, fontface="plain")+
  annotate("text", x = c(-2.5,2.5), y = c(25,25), label = c("sleep", "wake") , color=c("red","blue"), size=4, fontface="bold")
dev.off()

vol.O<-DEG.combine.all%>%
  mutate(class.y=ifelse(FDR.Y>0.01,"ns",ifelse(abs(aveFC.SDvSS.Y)>0.2,"high","low")),class.o=ifelse(FDR.O>0.01,"ns",ifelse(abs(aveFC.SDvSS.O)>0.2,"high","low")))%>%
  mutate_at(vars(starts_with("FDR")),funs(-log10(.)))%>%
ggscatter(.,x="aveFC.SDvSS.O",y="FDR.O",size=0.4,color="class.o", palette=c("purple3","gray50","gray80"),xlab=expression('log'[2]*'FC of SD vs.SS'), ylab=expression('-log'[10]*'FDR'),title="",xlim=c(-3.5,3.5))+
geom_hline(yintercept=2, linetype="longdash", color = "black",size=0.5)+
  geom_vline(xintercept=c(-0.2,0.2), linetype="longdash", color = c("red","blue"),size=0.5)+
  scale_x_continuous(breaks=c(-3,-2,-1,-0.2,0.2,1,2,3))+scale_y_continuous(breaks=c(0,2,10,20,30))+
  theme(legend.position = "none",axis.title=element_text(size=10), axis.text.x=element_text(angle = 60,hjust = 1,size=6),axis.text.y=element_text(size=6))+
  annotate("segment", x = -0.2, xend = -1.2, y = 30, yend = 30, colour = "red", size=0.5, alpha=0.6, arrow=arrow(length = unit(0.2, "cm")))+
  annotate("segment", x = 0.2, xend = 1.2, y = 30, yend = 30, colour = "blue", size=0.5, alpha=0.6, arrow=arrow(length = unit(0.2, "cm")))+
  annotate("text", x = c(-2.5,2.5), y = c(30,30), label = c("1254", "1278") , color=c("red","blue"), size=4, fontface="plain")+
  annotate("text", x = c(-2.5,2.5), y = c(25,25), label = c("sleep", "wake") , color=c("red","blue"), size=4, fontface="bold")


tiff(filename = "Figure1d.tiff",height =60,width =110,res = 800,units = "mm",type="cairo",compression="none")
ggarrange(vol.Y,vol.O,ncol=2)
 dev.off()
...
#DEGs with varying FC-cutoffs

test<-DEG.combine.all%>%
  dplyr::select(id,starts_with("FDR"),starts_with("aveFC"))%>%
  mutate(abs.aveFC.SDvSS.Y=abs(aveFC.SDvSS.Y),abs.aveFC.SDvSS.O=abs(aveFC.SDvSS.O))

library(VennDiagram)

venn.diagram(list(test[test$FDR.Y<0.01,]$id,test[test$FDR.O<0.01,]$id), "DEG.FC0.tiff", fill=brewer.pal(4,"Set1")[3:4], cex = 3, main="DEGs without FC-cutoff", main.pos=c(0.5,0.64), cat.fontface=2, margin=0.8,main.cex=3, cat.cex=3,category.names=c("Young","Old"),cat.pos=c(-50,55))

venn.diagram(list(test[test$FDR.Y<0.01&test$abs.aveFC.SDvSS.Y>0.1,]$id,test[test$FDR.O<0.01&test$abs.aveFC.SDvSS.O>0.1,]$id), "DEG.FC0.1.tiff", fill=brewer.pal(4,"Set1")[3:4], cex = 3, main="DEGs with log2FC>0.1", main.pos=c(0.5,0.64), cat.fontface=2, margin=0.8,main.cex=3, cat.cex=3,category.names=c("Young","Old"),cat.pos=c(-50,55))


venn.diagram(list(test[test$FDR.Y<0.01&test$abs.aveFC.SDvSS.Y>0.2,]$id,test[test$FDR.O<0.01&test$abs.aveFC.SDvSS.O>0.2,]$id), "DEG.FC0.2.tiff", fill=brewer.pal(4,"Set1")[3:4], cex = 3, main="DEGs with log2FC>0.2", main.pos=c(0.5,0.64), cat.fontface=2, margin=0.8,main.cex=3, cat.cex=3,category.names=c("Young","Old"),cat.pos=c(-50,55))


venn.diagram(list(test[test$FDR.Y<0.01&test$abs.aveFC.SDvSS.Y>0.5,]$id,test[test$FDR.O<0.01&test$abs.aveFC.SDvSS.O>0.5,]$id), "DEG.FC0.5.tiff", fill=brewer.pal(4,"Set1")[3:4], cex = 3, main="DEGs with log2FC>0.5", main.pos=c(0.5,0.64), cat.fontface=2, margin=0.8,main.cex=3, cat.cex=3,category.names=c("Young","Old"),cat.pos=c(-50,55))
...
Pval.compare.t<-DEG.combine.all%>%
  dplyr::select(id,dir,class,P.Value.Y,P.Value.O)%>%
  mutate_at(vars(starts_with("P.Value")),funs(-log10(.)))%>%
  filter(dir!="na")%>%
...
group_by(dir,class,deltaP.dir)%>%
  ...
  summarise(n=sum(n))%>%
  mutate(freq=n/sum(n)*100,class="combined")%>%
  dplyr::select(dir,class,deltaP.dir,n,freq)
...
group_by(dir,class,deltaP.dir)%>%
  summarise(n=n())%>%
  mutate(freq = n / sum(n)*100)%>%
  rbind(.,Pval.compare.t)%>%
filter(deltaP.dir=="O-down"&dir=="sleep")%>%
    ungroup(class)%>%
  mutate(class=factor(class,levels=c("common","Yspec","Ospec","combined"),labels=c("Common","Y-specific","O-specific","Combined")))%>%
  ggbarplot(x="class",y="freq",fill="class",palette = c("#00AFBB", "#E7B800", "#FC4E07","saddlebrown"),label=T,lab.pos="out",lab.size=4,lab.nb.digits=1,ylim=c(0,140), xlab="",ylab="%decrease in old")+theme(axis.title.y=element_text(size=8,face="bold"),axis.text.x=element_text(angle=25,hjust=1,size=7,face="bold"),axis.text.y=element_text(size=7),legend.position="none",plot.background = element_rect(fill = "transparent", color = NA))+
...
tiff(filename ="Figure3b.tiff",height = 5,width =8,res = 800,units = "in",type="cairo",compression = "zip")
ggarrange(p1,p2,ncol=2)
...
group_by(dir,class,deltaFC.dir)%>%
  summarise(n=n())%>%
  mutate(freq = n / sum(n)*100)%>%
  group_by(dir,deltaFC.dir)%>%
  summarise(n=sum(n))%>%
  mutate(freq=n/sum(n)*100,class="combined")%>%
  dplyr::select(dir,class,deltaFC.dir,n,freq)
...
ggscatter(.,x="aveFC.SDvSS.Y",y="deltaFC",size=1,color="class",palette = c("#00AFBB", "#E7B800", "#FC4E07"),ylim=c(-1.2,1.5), xlab=expression('Average log'[2]*'FC (SD/SS) of young mice'),ylab=expression('Difference in absolute average log'[2]*'FC (old-young)'),title="Wake Genes")+
    geom_hline(yintercept = 0,linetype="dashed",color="black")+
    theme(plot.title=element_text(size=16,face="bold",hjust=0.5),legend.position="none",axis.title=element_text(size=13,face="bold"))+
  annotation_custom(grob = bar4_grob, xmin = 1, xmax = 3, 
                       ymin = 0.3, ymax = 1.5)
...
#correlation between changes in pval and changes in FC

Pval.compare<-DEG.combine.all%>%
  dplyr::select(id,dir,class,P.Value.Y,P.Value.O)%>%
  mutate_at(vars(starts_with("P.Value")),funs(-log10(.)))%>%
  filter(dir!="na")%>%
  mutate(deltaP=P.Value.O-P.Value.Y)%>%
...
FC.compare<-DEG.combine.all%>%
  dplyr::select(id,dir,class,starts_with("aveFC.SDvSS"))%>%
  filter(dir!="na")%>%
  mutate(deltaFC=abs(aveFC.SDvSS.O)-abs(aveFC.SDvSS.Y))%>%
  mutate(deltaFC.dir=ifelse(deltaFC>0,"O-up","O-down"))%>%
  left_join(.,Pval.compare,by="id")

png(filename ="corr.deltaP.deltaFC.png",height = 3,width =3.5,res = 300,units = "in",type="cairo")
ggscatter(FC.compare,x="deltaP",y="deltaFC",color="dir.x",size=0.5,add="reg.line",conf.int=F,xlab=expression('delta abs(-log'[10]*'Pval)'),ylab=expression('delta abs(log'[2]*'FC)'))+
  stat_cor(method = "pearson", label.x = -10, label.y = 1)+
...
#compare y and o with mean fold change (2^convert to %expression change) for all the genes involved in each functional cluster
...
theme(legend.position=c(0.95,0.9),legend.title=element_blank(), axis.text.x = element_text(angle=35,hjust=1,size=8,face="bold",color=rep(c("black", "darkorange", "green4"),c(12,2,3))),  plot.title=element_text(face="bold",size=10,hjust=0.5), axis.title.y=element_text(face="bold",size=8))

dev.off()

tiff(filename ="Figure5.tiff",height = 4,width =8,res = 800,units = "in",type="cairo",compression="zip")
...
FC.FDRandFC%>%
  ...
  gather(age,aveFC,aveFC.SDvSS.Y:aveFC.SDvSS.O)%>%
  mutate(age=factor(age,levels=c("aveFC.SDvSS.Y","aveFC.SDvSS.O"),labels=c("Young","Old")),aveFC=(2^aveFC-1)*100, FCname=factor(FC,levels=c("w.c.ER", "w.c.trans","w.c.endo", "w.c.dephos", "w.c.circadian", "w.c.FoxO", "w.c.cAMP", "w.c.MAPK", "w.c.Kinase", "w.c.hormone", "w.c.Dioxygenase", "w.c.fatcell", "w.y.actin", "w.y.GTPase", "w.o.lipid", "w.o.trans", "w.o.Tcell "), labels=c("ERstress", "Reg.transcription","Response to VEGF", "Dephosphorylation", "RhythmicProcess", "FoxO sigaling", "Response to cAMP", "MAPK signaling", "Kinase activity", "Cytokine activity", "Dioxygenase", "Fat cell differentiation", "Actin bindng", "GTPase activation","Lipid transport", "Regu.transcription", "Tcell receptor")))%>%
  ggbarplot(x="FCname",y="aveFC",color="age",palette=brewer.pal(9,"Blues")[c(6,8)],size=1,width=0.5,position=position_dodge(width=0.65),add="mean_se",xlab="",ylab="Expresion (% change)",title="Expression changes between SD and SS for functions enriched among wake genes")+
  stat_compare_means(aes(group = age), paired=T,label="p.signif",label.y=100,hide.ns = TRUE)+
theme(legend.position=c(0.95,0.9),legend.title=element_blank(), axis.text.x = element_text(angle=35,hjust=1,size=8,face="bold",color=rep(c("black", "darkorange", "green4"),c(12,2,3))),  plot.title=element_text(face="bold",size=10,hjust=0.5), axis.title.y=element_text(face="bold",size=12))
...
sleep.p1<-rbind(as.data.frame(FC.FDRandFC),as.data.frame(transmembrane))%>%
  ...
  ggbarplot(x="FCname",y="pos.FC",color="age",palette=brewer.pal(9,"Reds")[c(6,9)],size=1.5,width=0.5,position=position_dodge(width=0.65),ylim=c(0,40),add="mean_se",xlab="",ylab="Expression (% change)",title="Expression changes between SS and SD for functions enriched among sleep genes")+
    geom_hline(yintercept = 0,linetype="dashed")+
  stat_compare_means(aes(group = age), paired=T,label="p.signif",label.y=35,hide.ns = TRUE)+
theme(legend.direction = "horizontal", legend.position=c(0.85,0.95), legend.title=element_blank(),axis.text.x = element_text(angle=35,hjust=1,size=10,face="bold",color=rep(c("black", "darkorange", "green4","blue"),c(3,5,3,1))), plot.title=element_text(face="bold",size=12,hjust=0.5), axis.title.y=element_text(face="bold",size=12))
...
sleep.p2<-rbind(as.data.frame(FC.FDRandFC),as.data.frame(transmembrane))%>%
  ...
  ggbarplot(x="FCname",y="aveFC",color="age",palette=brewer.pal(9,"Reds")[c(6,9)],size=1.5,width=0.5,position=position_dodge(width=0.65),add="mean_se",ylim=c(-10,40),xlab="",ylab="Expression (% change)",title="Expression changes between SS and ZT0 for functions enriched among sleep genes")+
...
tiff(filename ="Figure4.tiff",height = 8,width =8,res = 800,units = "in",type="cairo",compression="zip")
...
FC.meanFC.pairedwilcox<-rbind(as.data.frame(FC.FDRandFC),as.data.frame(transmembrane))%>%
  ...
  left_join(functioncluser.meanFC,.,by="FC")%>%
  mutate(FCname=factor(FC,levels=c("w.c.ER", "w.c.trans","w.c.endo", "w.c.dephos", "w.c.circadian", "w.c.FoxO", "w.c.cAMP", "w.c.MAPK", "w.c.Kinase", "w.c.hormone", "w.c.Dioxygenase", "w.c.fatcell", "w.y.actin", "w.y.GTPase", "w.o.lipid", "w.o.trans", "w.o.Tcell ","s.c.DNA", "s.c.Signal", "s.c.Gprotein", "s.y.adhesion", "s.y.axon", "s.y.Signal", "s.y.kinase", "s.y.chloride", "s.o.Glycoprotein", "s.o.Transmem", "s.o.Calcium", "s.chol", "s.translation","Transmem.combined"), labels=c("ERstress", "Reg.transcription","Response to VEGF", "Dephosphorylation", "RhythmicProcess", "FoxO sigaling", "Response to cAMP", "MAPK signaling", "Kinase activity", "Cytokine activity", "Dioxygenase", "Fat cell differentiation", "Actin bindng", "GTPase activation","Lipid transport", "Regu.transcription", "Tcell receptor","DNA repair", "Signal/transmembrane", "Gprotein receptor", "Cell adhesion", "Axon guidance", "Signal/Transmembrane", "Kinase activity", "Chloride channel", "Collagen/glycoprotein", "Transmembrane", "Calcium transport", "Cholesterol synthesis", "Translation","Transmem.combined")))%>%
  rename(SDvSS.pval=SDvSS,SSvZ0.pval=SSvZ0)


write.table(FC.meanFC.pairedwilcox,"FC.meanFC.pairedwilcox.txt",sep="\t",row.names = FALSE)


#calculate effect size of mean differences between young and old


FC.effectsize<-rbind(as.data.frame(FC.FDRandFC),as.data.frame(transmembrane))%>%
  filter(EC!="na")%>%
  dplyr::select(FC,class,id, starts_with("aveFC.SDvSS"), aveYSS.vZ0,aveOSS.vZ0)%>%
  gather(con,aveFC,aveFC.SDvSS.Y:aveOSS.vZ0)%>%
  mutate(age=ifelse(grepl("Y",con),"young","old"),comparison=ifelse(grepl("SDvSS",con),"SDvSS","SSvZ0"))%>%
  dplyr::select(-con)%>%
  group_by(FC,age,comparison)%>%
  summarise(meanlogFC=mean(aveFC),sdlogFC=sd(aveFC),meanFC=mean(2^aveFC),sdFC=sd(2^aveFC))%>%
  unite(con,FC,comparison,sep="&")%>%
  gather(measure,value,-(con:age))%>%
  unite(age.measure,age,measure,sep=".")%>%
  spread(age.measure,value)%>%
  mutate(es.meanFC=(old.meanFC-young.meanFC)/sqrt((young.sdFC^2+old.sdFC^2)/2),es.meanlogFC=(old.meanlogFC-young.meanlogFC)/sqrt((young.sdlogFC^2+old.sdlogFC^2)/2))%>% #use the effect size calculated using log2FC
  mutate(es.meanlogFC=abs(es.meanlogFC))%>% #take absolute value for effect size
  dplyr::select(con,es.meanlogFC)%>%
  separate(con,c("FC","comparison"),sep="&")%>%
  spread(comparison,es.meanlogFC)%>%
  rename(SDvSS.ES=SDvSS,SSvZ0.ES=SSvZ0)%>%
  left_join(FC.meanFC.pairedwilcox,.,by="FC")

write.table(FC.effectsize,"FC.effectsize.txt",sep="\t",row.names = FALSE)
...
c.sc.SDvSS.comb%>%
  ...
  t%>%
  pheatmap(., breaks = colors.camera, color=my_palette.camera, cluster_cols=FALSE,cluster_rows=FALSE,border_color= NA,annotation_row =annotation.row, annotation_col=annotation.col, gaps_row=4,cellwidth = 35, cellheight = 32, gaps_col=12,labels_row=c("ZT3","ZT6","ZT9","ZT12","ZT3","ZT6","ZT9","ZT12"),fontsize = 14.5,legend_breaks=c(-5,-1.3,1.3,5),legend_labels =c("e-5","0.05","0.05","e-5"), main="(b) Enrichment of cell-type specific gene sets in sleep and wake regulation")

dev.off()

png(filename = "Figure6b.png",height = 5.7,width =10,res = 800,units = "in",type="cairo")

c.sc.SDvSS.comb%>%
  dplyr::select(-celltype)%>%
  column_to_rownames(.,var="sc.geneset.n")%>%
  t%>%
  pheatmap(., breaks = colors.camera, color=my_palette.camera, cluster_cols=FALSE,cluster_rows=FALSE,border_color= NA,annotation_row =annotation.row, annotation_col=annotation.col, gaps_row=4,cellwidth = 30, cellheight = 25, gaps_col=12,labels_row=c("ZT3","ZT6","ZT9","ZT12","ZT3","ZT6","ZT9","ZT12"),fontsize = 12,legend_breaks=c(-5,-1.3,1.3,5),legend_labels =c("e-5","0.05","0.05","e-5"), main="  Enrichment of cell-type specific gene sets in sleep and wake regulation")
...
c.sc.OvY.comb%>%
  ...
  column_to_rownames(.,var="sc.geneset.n")%>%
  t%>%
  pheatmap(., breaks = colors.camera, color=my_palette.camera, cluster_cols=FALSE,cluster_rows=FALSE,border_color= NA, annotation_col=annotation.col, gaps_col=12, labels_row=c("ZT0","ZT3","ZT6","ZT9","ZT12"),cellwidth = 35, cellheight = 35, fontsize = 14.5,fontsize_col=15,fontsize_row=14, legend_breaks=c(-8,-1.3,1.3,8),legend_labels =c("e-8","0.05","0.05","e-8"), main="(a) Enrichment of cell-type specific gene sets in young and old comparison")

dev.off()

png(filename = "Figure6a.png",height = 5,width =10,res = 800,units = "in",type="cairo")

c.sc.OvY.comb%>%
  dplyr::select(sc.geneset.n,dir.P.ZT0,starts_with("dir.P.SS"))%>%
  column_to_rownames(.,var="sc.geneset.n")%>%
  t%>%
  pheatmap(., breaks = colors.camera, color=my_palette.camera, cluster_cols=FALSE,cluster_rows=FALSE,border_color= NA, annotation_col=annotation.col, gaps_col=12, labels_row=c("ZT0","ZT3","ZT6","ZT9","ZT12"),cellwidth = 30, cellheight = 25, fontsize = 12,fontsize_col=15,fontsize_row=14, legend_breaks=c(-8,-1.3,1.3,8),legend_labels =c("e-8","0.05","0.05","e-8"), main="    Enrichment of cell-type specific gene sets in young and old comparison")
...
syn.home<-c("ENSMUSG00000021250","ENSMUSG00000003545","ENSMUSG00000038418","ENSMUSG00000037868","ENSMUSG00000022602","ENSMUSG00000048482","ENSMUSG00000055254","ENSMUSG00000020524","ENSMUSG00000059003","ENSMUSG00000025892","ENSMUSG00000039579","ENSMUSG00000004113","ENSMUSG00000040118","ENSMUSG00000063887","ENSMUSG00000061601","ENSMUSG00000037217","ENSMUSG00000009394","ENSMUSG00000024109","ENSMUSG00000066392","ENSMUSG00000031558","ENSMUSG00000042453","ENSMUSG00000022538","ENSMUSG00000020932","ENSMUSG00000023828")
...
gather(sampleID,CPM,-geneSymbol)%>%
  mutate(age=ifelse(grepl("Young",sampleID),"Y","O"), sleep=ifelse(grepl("SS",sampleID),"SS","SD"),
         time=ifelse(grepl("Z0",sampleID),"0",ifelse(grepl("3hr",sampleID),"3",ifelse(grepl("6hr",sampleID),"6",ifelse(grepl("9hr",sampleID),"9","12")))))%>%
     unite(age.sleep,age,sleep,sep = ".",remove=F)%>%
  mutate(geneSymbol=factor(geneSymbol,levels=c("Fos","Fosb","Egr1","Egr2","Arc","Bdnf","Ntrk2","Gria1","Grin2a","Gria4","Grin3a","Cacna1b","Cacna2d1","Nlgn1","Pclo","Syn1","Syn2","Nrxn1","Nrxn3","Slit2","Reln","Lsg1","Gfap","Slc22a3")),age.sleep=factor(age.sleep,levels=c("Y.SS","O.SS","Y.SD","O.SD")))
...
SH.figure<-ggarrange(SH.plot(1),SH.plot(2),SH.plot(3),SH.plot(4),SH.plot(5),SH.plot(6),SH.plot(7),SH.plot(8),SH.plot(9),SH.plot(10),SH.plot(11),SH.plot(12),SH.plot(13),SH.plot(14),SH.plot(15),SH.plot(16),SH.plot(17),SH.plot(18),SH.plot(19),SH.plot(20),SH.plot(21),SH.plot(22),SH.plot(23),SH.plot(24),ncol = 4, nrow =6,common.legend = T, legend ="top")

png(filename = "synaptichome.png",height = 13,width =8,res = 300,units = "in",type="cairo")
annotate_figure(SH.figure,top = text_grob("Synaptic homeostasis related genes", color = "black", face = "bold", size = 20),bottom = text_grob("Zeitgeber time (ZT)", color = "black",hjust = 0.5, face = "bold", size = 18),left = text_grob("Gene expression (logCPM)", color = "black", face = "bold",rot = 90, size = 20),fig.lab.size=20,fig.lab.face="bold")
dev.off()


test<-cbind(efit.main$genes,as.data.frame(efit.main$p.value))%>%
  filter(id%in%syn.home)%>%
  dplyr::select(geneSymbol,starts_with("OvY"))
...
#5-29-19; recreate figure for publication

vol.SS<-OvY.combine.result%>%
  mutate(class.SS=ifelse(FDR.SS>0.01,"ns",ifelse(abs(aveFC.OvY.SS)>0.2,"high","low")),class.Z0=ifelse(FDR.ZT0>0.01,"ns",ifelse(abs(OvY.ZT0)>0.2,"high","low")))%>%
  mutate_at(vars(starts_with("FDR")),funs(-log10(.)))%>%
ggscatter(.,x="aveFC.OvY.SS",y="FDR.SS",size=0.4,color="class.SS", palette=c("sienna2","gray50","gray80"),xlab=expression('log'[2]*'FC of O vs.Y'), ylab=expression('-log'[10]*'FDR'),title="",xlim=c(-5,5))+
geom_hline(yintercept=2, linetype="longdash", color = "black",size=0.5)+
  geom_vline(xintercept=c(-0.2,0.2), linetype="longdash", color = c("green4","red"),size=0.5)+
  scale_x_continuous(breaks=c(-4,-2,-0.2,0.2,2,4))+scale_y_continuous(breaks=c(0,2,10,20,30))+
  theme(legend.position = "none",axis.title=element_text(size=10), axis.text.x=element_text(angle = 60,hjust = 1,size=6),axis.text.y=element_text(size=6))+
  annotate("segment", x = -0.2, xend = -1, y = 24, yend = 24, colour = "green4", size=0.5, alpha=0.5, arrow=arrow(length = unit(0.2, "cm")))+
  annotate("segment", x = 0.2, xend = 1, y = 24, yend = 24, colour = "red", size=0.5, alpha=0.5, arrow=arrow(length = unit(0.2, "cm")))+
  annotate("text", x = c(-3.3,3), y = c(24,24), label = c("195", "341") , color=c("green4","red"), size=4)+
  annotate("text", x = c(-3.3,3), y = c(20,20), label = c("Down", "Up") , color=c("green4","red"), size=4,fontface="bold")

vol.Z0<-OvY.combine.result%>%
  mutate(class.SS=ifelse(FDR.SS>0.01,"ns",ifelse(abs(aveFC.OvY.SS)>0.2,"high","low")),class.Z0=ifelse(FDR.ZT0>0.01,"ns",ifelse(abs(OvY.ZT0)>0.2,"high","low")))%>%
  mutate_at(vars(starts_with("FDR")),funs(-log10(.)))%>%
ggscatter(.,x="OvY.ZT0",y="FDR.ZT0",size=0.4,color="class.Z0", palette=c("purple","gray50","gray80"),xlab=expression('log'[2]*'FC of O vs.Y'), ylab=expression('-log'[10]*'FDR'),title="",xlim=c(-5,5))+
geom_hline(yintercept=2, linetype="longdash", color = "black",size=0.5)+
  geom_vline(xintercept=c(-0.2,0.2), linetype="longdash", color = c("green4","red"),size=0.5)+
  scale_x_continuous(breaks=c(-4,-2,-0.2,0.2,2,4))+scale_y_continuous(breaks=c(0,2,5,10,15))+
  theme(legend.position = "none",axis.title=element_text(size=10), axis.text.x=element_text(angle = 60,hjust = 1,size=6),axis.text.y=element_text(size=6))+
  annotate("segment", x = -0.2, xend = -1, y = 11, yend = 11, colour = "green4", size=0.5, alpha=0.5, arrow=arrow(length = unit(0.2, "cm")))+
  annotate("segment", x = 0.2, xend = 1, y = 11, yend = 11, colour = "red", size=0.5, alpha=0.5, arrow=arrow(length = unit(0.2, "cm")))+
  annotate("text", x = c(-3.3,3), y = c(11,11), label = c("108", "144") , color=c("green4","red"), size=4)+
  annotate("text", x = c(-3.3,3), y = c(9,9), label = c("Down", "Up") , color=c("green4","red"), size=4,fontface="bold")


tiff(filename ="Figure1f.tiff",height = 60,width =110,res = 800,units = "mm",type="cairo",compression="none")
ggarrange(vol.SS,vol.Z0,ncol=2)
   dev.off()    
...
# 5-29-19; redo figure for publication
venn.diagram(list(OvY.combine.result[OvY.combine.result$FDR.ZT0<0.01,]$id,OvY.combine.result[OvY.combine.result$FDR.SS<0.01,]$id), "Figure1e.tiff", fill=brewer.pal(5,"Set1")[4:5], cex = 3.5, main="", main.pos=c(0.5,0.64), cat.fontface=2, margin=0.85,main.cex=2.6, cat.cex=3,category.names=c("ZT0","SS"),cat.pos=c(-140,120))
...
# temporal plots with loess regression lines; 5-30-19 replot for publication 

exp.time.YZT0<-DEG.combine.all%>%
  dplyr::select(id,sleep,wake,dir)%>%
  filter(dir!="na")%>%
  ...
  dplyr::select(-Z0.OvY,-contains("vYZ0.O"))%>%
  gather(con,FCvZ0,SSvYZ0.Y3h:SDvOZ0.O0h)%>%
...
state=ifelse(grepl("SS",con),"SS","SD"),age=ifelse(grepl(".Y",con),"Young","Old"))%>%
    mutate(FCvZ0.pc=((2^(FCvZ0)-1)*100))%>% #convert log2FC to % change relative to ZT0
  mutate(condition=factor(condition,levels=c("Y.SS","O.SS","Y.SD","O.SD")),sleep=factor(sleep,levels=c("Yspec","common","Ospec","na"),labels=c("Y-specific","Common","O-specific","na")), wake=factor(wake,levels=c("Yspec","common","Ospec","na"),labels=c("Y-specific","Common","O-specific","na")), state=factor(state,levels=c("SS","SD")), age=factor(age,levels=c("Young","Old")))
...
tiff(filename ="exp.agevsclass.tiff",height = 4,width =8,res = 800,units = "in",type="cairo",compression="zip")
...
p3<- exp.time.YZT0%>%
  filter(dir=="sleep")%>%
ggscatter(.,x="time",y="FCvZ0.pc",point=F,color="condition",palette =exp.palette,add="loess",conf.int=T, conf.int.level=0.95, xlab="Zeitgeber time (ZT)", ylab="Relative Expression Change (%)")+
  facet_grid(state~sleep)+
scale_x_continuous(breaks=c(0,3,6,9,12))+theme(legend.position = "bottom",legend.title =element_blank(),strip.background.x = element_rect(fill = brewer.pal(9,"YlOrBr")[8]),strip.background.y = element_rect(fill ="white"),strip.text.x = element_text(colour = "white", face = "bold"),strip.text.y = element_text(colour = "black", face = "bold"),panel.background = element_rect(colour = "black"), axis.title=element_text(size=12,face="bold"))+ geom_hline(yintercept = 0,color="black",linetype="longdash",size=0.5)
  
p4<-exp.time.YZT0%>%
  filter(dir=="wake")%>%
ggscatter(.,x="time",y="FCvZ0.pc",point=F,color="condition",palette =exp.palette,add="loess",conf.int=T, conf.int.level=0.95, xlab="Zeitgeber time (ZT)", ylab="Relative Expression Change (%)")+
  facet_grid(state~wake)+
scale_x_continuous(breaks=c(0,3,6,9,12))+theme(legend.position = "bottom",legend.title =element_blank(),strip.background.x = element_rect(fill = brewer.pal(9,"PuBu")[8]),strip.background.y = element_rect(fill ="white"),strip.text.x = element_text(colour = "white", face = "bold"),strip.text.y = element_text(colour = "black", face = "bold"),panel.background = element_rect(colour = "black"), axis.title=element_text(size=12,face="bold"))+ geom_hline(yintercept = 0,color="black",linetype="longdash",size=0.5)

  
tiff(filename ="exp.statevsclass.tiff",height = 4,width =8,res = 800,units = "in",type="cairo",compression="zip")
ggarrange(p3,p4,ncol=2,common.legend = T, legend="bottom")

dev.off()


#6-11-19; temporal change of SD vs. SS fold changes for young and old mice



test<-DEG.combine.all%>%
  filter(dir!="na")%>%
  mutate(group=ifelse(sleep=="common"|wake=="common","Common",ifelse(sleep=="Yspec"|wake=="Yspec","Y-specific","O-specific")),SDvsSS_0hr.Y=0,SDvsSS_0hr.O=0)%>%
  dplyr::select(id,group,dir,starts_with("SDvsSS"))%>%
  gather(con,FC,SDvsSS_3hr.Y:SDvsSS_0hr.O)%>%
  mutate(age=ifelse(grepl(".Y",con),"Young","Old"),time=ifelse(grepl("3hr",con),3,ifelse(grepl("6hr",con),6,ifelse(grepl("9hr",con),9,ifelse(grepl("12hr",con),12,0)))))%>%
    mutate(FC.pc=((2^(FC)-1)*100))%>% #convert log2FC to % change of SD relative to SS
  mutate(group=factor(group,levels=c("Y-specific","Common","O-specific")), 
         age=factor(age,levels=c("Young","Old")))

p5<-test%>%
  filter(dir=="sleep")%>%
  mutate(FC.pc=FC.pc*(-1))%>%
ggscatter(.,x="time",y="FC.pc",point=F,color="age",conf.int=F,add="loess", facet.by="group",palette=c("green","purple"), conf.int.level=0.95, xlab="Zeitgeber time (ZT)", ylab="%Change of SS/SD")+
scale_x_continuous(breaks=c(0,3,6,9,12))+theme(legend.position = "bottom",legend.title =element_blank(),strip.background.x = element_rect(fill = brewer.pal(9,"YlOrBr")[8]),strip.background.y = element_rect(fill ="white"),strip.text.x = element_text(colour = "white", face = "bold"),strip.text.y = element_text(colour = "black", face = "bold"),panel.background = element_rect(colour = "black"), axis.title=element_text(size=12,face="bold"))+ geom_hline(yintercept = 0,color="black",linetype="longdash",size=0.5)
  

p6<-test%>%
  filter(dir=="wake")%>%
ggscatter(.,x="time",y="FC.pc",point=F,color="age",conf.int=F,add="loess", facet.by="group",palette=c("green","purple"), conf.int.level=0.95, xlab="Zeitgeber time (ZT)", ylab="%Change of SD/SS")+
scale_x_continuous(breaks=c(0,3,6,9,12))+theme(legend.position = "bottom",legend.title =element_blank(),strip.background.x = element_rect(fill = brewer.pal(9,"PuBu")[8]),strip.background.y = element_rect(fill ="white"),strip.text.x = element_text(colour = "white", face = "bold"),strip.text.y = element_text(colour = "black", face = "bold"),panel.background = element_rect(colour = "black"), axis.title=element_text(size=12,face="bold"))+ geom_hline(yintercept = 0,color="black",linetype="longdash",size=0.5)
  

tiff(filename ="exp.SDvSS.tiff",height = 2.5,width =8,res = 800,units = "in",type="cairo",compression="zip")
ggarrange(p5,p6,ncol=2,common.legend = T, legend="bottom")
...
#5-30-19; redo figure for publication
  
p1<-DEG.combine.all%>%
  filter(sleep!="na")%>%
  dplyr::select(id,sleep,P.Value.Y,P.Value.O)%>%
  mutate_at(vars(starts_with("P.Value")),funs(-log10(.)))%>%
  gather(age,p.val,P.Value.Y:P.Value.O)%>%
  mutate(age=ifelse(grepl("Y",age),"Young","Old"))%>%
mutate(age=factor(age,levels=c("Young","Old")),sleep=factor(sleep,levels=c("common","Yspec","Ospec"),labels=c("Common","Y-specific","O-specific")))%>%
  ggviolin(.,x="age",y="p.val",fill = "sleep",palette = c("#00AFBB", "#E7B800", "#FC4E07"), add="boxplot",add.params = list(fill = "white"), xlab="",ylab="-log10 (P.Value)",title="Sleep Genes")+
  facet_wrap(~sleep,strip.position = "bottom")+
  stat_compare_means(label="p.signif",paired = TRUE,label.x=1.4,label.y=40)+
theme(legend.position="none",strip.text = element_text(face="bold",size=12,color="white"), axis.text.x = element_text(face="bold",size=11), axis.title=element_text(size=12), strip.background = element_rect(color="white",fill =brewer.pal(9,"YlOrBr")[8]), plot.title=element_text(face="bold",size=14,hjust=0.5))+
  scale_y_continuous(expand=c(0,1))

p2<-DEG.combine.all%>%
  filter(wake!="na")%>%
  dplyr::select(id,wake,P.Value.Y,P.Value.O)%>%
  mutate_at(vars(starts_with("P.Value")),funs(-log10(.)))%>%
  gather(age,p.val,P.Value.Y:P.Value.O)%>%
  mutate(age=ifelse(grepl("Y",age),"Young","Old"))%>%
mutate(age=factor(age,levels=c("Young","Old")),wake=factor(wake,levels=c("common","Yspec","Ospec"),labels=c("Common","Y-specific","O-specific")))%>%
  ggviolin(.,x="age",y="p.val",fill = "wake",palette = c("#00AFBB", "#E7B800", "#FC4E07"), add="boxplot",add.params = list(fill = "white"), xlab="",ylab="-log10 (P.Value)",title="Wake Genes")+
  facet_wrap(~wake,strip.position = "bottom")+
  stat_compare_means(label="p.signif",paired = TRUE,label.x=1.4,label.y=40)+
theme(legend.position="none",strip.text = element_text(face="bold",size=12,color="white"), axis.text.x = element_text(face="bold",size=11), axis.title=element_text(size=12), strip.background = element_rect(color="white",fill =brewer.pal(9,"PuBu")[8]), plot.title=element_text(face="bold",size=14,hjust=0.5))+
  scale_y_continuous(expand=c(0,1))
  
  
tiff(filename ="Figure3a.tiff",height = 4,width =8,res = 800,units = "in",type="cairo",compression="zip")
